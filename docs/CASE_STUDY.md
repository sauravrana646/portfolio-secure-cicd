...
5. Manual semver tag release from `main`: GitHub Environment `release` gate (required reviewer + `v*` tags only), then push to GHCR by tag + digest, sign and attest with cosign (private key via Infisical OIDC), attach Syft SPDX SBOM attestation, emit SLSA build provenance, publish a GitHub Release with changelog
